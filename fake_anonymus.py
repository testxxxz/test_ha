...
BOT_TOKEN = '7273480329:AAFHUcRKsfwpdQevPa4KH0Qpe1mW8KWiEKw'  # Replace with your actual bot token
...
welcome_msg="""در حال ارسال پیام ناشناس به I'm Hani هستی.
...
# Admin ID
id_hani = '2030346115'  # Replace with your actual admin chat ID
...
def handle_text(message):
    try:
        ...
        if message.text in  filter_msgs:
            ...
            bot.send_message(id_hani, f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_photo(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_photo(id_hani, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_video(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_video(id_hani, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_voice(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_voice(id_hani, file_id, caption=f'Message from @{user_name} ({user_id}): Voice Message', reply_markup=create_markup(user_id, message.message_id))
...
def handle_document(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_document(id_hani, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_audio(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_audio(id_hani, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_callback(call):
    try:
        ...
        if action == 'respond':
            msg = bot.send_message(id_hani, "Please type your response:")
            ...
            bot.send_message(id_hani, f'Message from {user_id} was deleted.')
    except Exception as e:
...
def process_response(message, user_id):
    try:
        ...
        bot.send_message(id_hani, f'Response sent to {user_id}.')
