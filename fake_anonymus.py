...
def view_messages(message):
    try:
        if str(message.chat.id) == id_hani:
            if user_messages:
                for idx, (user_id, user_name, text) in enumerate(user_messages):
                    ...
                    markup.add(btn)
                    bot.send_message(id_hani, f'Message from @{user_name} ({user_id}): {text}', reply_markup=markup)
            else:
                bot.send_message(id_hani, "No messages to display.")
        else:
...
@bot.message_handler(func=lambda message: message.chat.id != int(id_hani))
...
def handle_response(call):
    try:
        if str(call.message.chat.id) == id_hani:
...
def process_response(message, user_id, user_name, idx):
    try:
        ...
        bot.send_message(id_hani, f'Response sent to @{user_name} ({user_id}).')
...
def clear_messages(message):
    try:
        if str(message.chat.id) == id_hani:
            user_messages.clear()
            bot.send_message(id_hani, "All messages have been cleared.")
        else:
            bot.reply_to(message, "This command is for admin only.")
    except Exception as e:
